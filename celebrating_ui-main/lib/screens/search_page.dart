...
class _SearchPageState extends State<SearchPage>
  ...
  void _performSearch(String query) async {
    ...
    switch (_tabController.index) {
      case 0:
        results = await SearchService.searchUsers(query, token: token);
        setState(() {
        ...
        break;
      case 1:
        results = await SearchService.searchFlicks(query, token: token);
        setState(() {
      ...
      case 2:
        ...
        break;
      case 3:
        results = await SearchService.searchAudio(query, token: token);
        setState(() {
  ...
  Widget _buildPeopleTab() {
    ...
    return ListView.builder(
      itemCount: _searchUserResults.length,
      itemBuilder: (context, i) => SearchUserCard(user: _searchUserResults[i]),
    );
  }
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (notification) {
        if (notification is ScrollUpdateNotification ||
          ...
          final itemHeight =
              itemWidth / 0.6 + 3.0; // childAspectRatio and spacing
